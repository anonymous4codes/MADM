...
import random
from BaseModel import DiffNet_plus, DESIGN, GraphRec
...
class MyModel(nn.Module):
    ...
    def calculate_loss(self, users, pos, neg, epoch):
        ...
        # ssl_loss = self.graphlearner.ssl_loss(users, all_user_embs_1, all_user_embs_2)
        recon_loss = self.graphlearner.reconstru_loss(all_user_embs_1, self.RRTdrop)
        if self.base_model_name == 'DiffNet++' or self.base_model_name == 'GraphRec':
        ...
        if self.base_model_name == 'DESIGN':
            ...
            kl_loss = self.basemodel.KL_loss(users_emb, pos_emb, neg_emb, users_emb_social, pos_emb_social,\
        ...
        if self.base_model_name == 'DiffNet++' or self.base_model_name == 'GraphRec':
            # loss = bpr_loss+self.decay*reg_loss+ self.ssl_reg*ssl_loss
            loss = bpr_loss+self.decay*reg_loss+  self.recon_reg*recon_loss
        if self.base_model_name == 'DESIGN':
            # loss = bpr_loss+self.decay*reg_loss + self.kl_reg*kl_loss+self.ssl_reg*ssl_loss
            loss = bpr_loss+self.decay*reg_loss+ self.kl_reg*kl_loss+  self.recon_reg*recon_loss
        return loss
