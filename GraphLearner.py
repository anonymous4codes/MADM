...
class GraphLearner(nn.Module):
    ...
    def forward(self, feature, init_graph):
        if self.metric_type == 'weighted_cosine':
            ...
            context_norm = F.normalize(context_fc, p=2, dim=-1) #  v/|v|^2 
            attention = torch.matmul(context_norm, context_norm.transpose(-1, -2)).mean(0) # BB^T=[4, N, feat_dim][4, feat_dim, N]->[N, N] similarity
            maskoff_value = 0   
    ...
    def ssl_loss(self, users, all_user_embs_1, all_user_embs_2):
        ...
        pos_ratings_user = torch.sum(
            user_1_embs*user_2_embs, dim=-1)  # [B] 

        tot_rating_user = torch.matmul(user_1_embs,
                                       torch.transpose(all_user_embs_2, 0, 1))  # [B,d]*[d, N]=[B，N] 
        ssl_logits_user = tot_rating_user - \
